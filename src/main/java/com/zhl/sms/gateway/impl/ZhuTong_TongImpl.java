 ...
 * @since 2015-8-28
 */
public class ZhuTong_TongImpl implements SmsPortal{
	
	private Logger logger = Logger.getLogger(ZhuTong_TongImpl.class);
...
//		StringBuilder sb = new StringBuilder().append("【中互联】").append(content);
		...
		logger.info("请求：【助通_通知】发送短信(" + service + ")接口【服务编号】:"+keySn+"【请求参数】：" + map.toString());
		long startTime = System.currentTimeMillis();
		String s = SmsHttpUtil.httpsPost(url + service, "", map);
		logger.info("耗时：【助通_通知】(" + service + ")接口【服务编号】:"+keySn+"耗时:"+(System.currentTimeMillis()-startTime)+"毫秒");
		if (s.contains("1,")) {
			logger.info("响应：【助通_通知】(" + service + ")接口【服务编号】:"+keySn+"【返回参数】：" + s + ",发送短信成功");
		} else {
			logger.info("响应：【助通_通知】(" + service + ")接口【服务编号】:"+keySn+"【返回参数】：" + s);
		}
		return s;
	}
	...
	public String getUserInfo(String keySn, String username, String password, String url) {
		...
		logger.info("请求：【助通_通知】余额查询(" + service + ")接口【服务编号】:"+keySn+"【请求参数】：" + map.toString());
		long startTime = System.currentTimeMillis();
		String result = SmsHttpUtil.httpsPost(url + service, "", map);
		logger.info("耗时：【助通_通知】(" + service + ")接口【服务编号】:"+keySn+"耗时:"+(System.currentTimeMillis()-startTime)+"毫秒");
		logger.info("响应：【助通_通知】(" + service + ")接口【服务编号】:"+keySn+"【返回参数】：" + result);
	...
	public String batchSend(String keySn, String username, String password, String url, String phone, String content, String sendTo, String type, String notifyUrl, String userReplyUrl) {
		...
		map.put("mobile", phone);
		map.put("content", content);
		logger.info("请求：【助通_通知】批量发送短信(" + service + ")接口【服务编号】:"+keySn+"【请求参数】：" + map.toString());
		long startTime = System.currentTimeMillis();
//		String s = "1,201702211023094031145";
		String s = SmsHttpUtil.httpsPost(url + service, "", map);
		logger.info("耗时：【助通_通知】(" + service + ")接口【服务编号】:"+keySn+"耗时:"+(System.currentTimeMillis()-startTime)+"毫秒");
		if (s.contains("1,")) {
			logger.info("响应：【助通_通知】(" + service + ")接口【服务编号】:"+keySn+"【返回参数】：" + s + ",发送短信成功");
	...
	public Map<String, Object> batchIdentitySend(String keySn, String username, String password, String url, String phone, String content) {
		...
		map.put("mobile", phone);
		map.put("content", content);
		logger.info("请求：【助通_通知】批量个性化短信(" + service + ")接口【服务编号】:"+keySn+"【请求参数】：" + map.toString());
		long startTime = System.currentTimeMillis();
		String s = SmsHttpUtil.httpsPost(url + service, "", map);
		logger.info("耗时：【助通_通知】(" + service + ")接口【服务编号】:"+keySn+"耗时:"+(System.currentTimeMillis()-startTime)+"毫秒");
		String result = "ok";
